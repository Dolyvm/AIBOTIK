...
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build # пересобрать и перезапустить
```

### Одной командой с локальной машины

```bash
ssh root@161.35.153.41 "cd /root/AIBOTIK && git stash && git pull origin production && git stash pop && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build"
```
...
```bash
# Только бот
ssh root@161.35.153.41 "cd /root/AIBOTIK && git stash && git pull origin production && git stash pop && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build bot"

# Только бэкенд
ssh root@161.35.153.41 "cd /root/AIBOTIK && git stash && git pull origin production && git stash pop && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build backend"

# Только фронтенд
ssh root@161.35.153.41 "cd /root/AIBOTIK && git stash && git pull origin production && git stash pop && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build frontend"
```
...
```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml restart             # перезапустить все
docker compose -f docker-compose.yml -f docker-compose.prod.yml restart bot         # перезапустить только бота
docker compose -f docker-compose.yml -f docker-compose.prod.yml restart backend     # перезапустить только бэкенд
docker compose -f docker-compose.yml -f docker-compose.prod.yml restart worker      # перезапустить только воркер
```

### Остановка и запуск

```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml stop                # остановить все
docker compose -f docker-compose.yml -f docker-compose.prod.yml start               # запустить все
docker compose -f docker-compose.yml -f docker-compose.prod.yml down                # остановить и удалить контейнеры
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d               # запустить (без пересборки)
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build       # пересобрать и запустить
```
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs bot -f --tail 50
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs backend -f --tail 50
```
...
```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d  # (или docker compose restart)
```
...
docker compose -f docker-compose.yml -f docker-compose.prod.yml restart bot             # перезапустить
```
